...
int maxPulsesIndex = 2400;
int maxPulsesMiddleRing = 1800;
int maxPulsesPinky = 2000;
...
void requestEvent() {
  int aPosition = (int) (maxPulsesPinky * percentage);
  int bcPosition = (int) (maxPulsesMiddleRing * percentage);
  int dPosition = (int) (maxPulsesIndex * percentage);

  bool aComplete = abs(numPulsesA - aPosition) < 20;
  bool bComplete = abs(numPulsesB - bcPosition) < 20;
  bool cComplete = abs(numPulsesC - bcPosition) < 20;
...
void setPosition() {
  ...
  if (abs(numPulsesA - aPosition) < 20) {
    Set_PWMA(0);
    // Serial.println();
  }
  ...
  if (abs(numPulsesB - bcPosition) < 20) {
    Set_PWMB(0);
    // Serial.println();
  }
  else if (numPulsesB > bcPosition) {
    Set_PWMB(-SPEED);
  }
  //numPulsesC < minPulses
  else {
  ...
  if (abs(numPulsesC - bcPosition) < 20) {
    Set_PWMC(0);
  }
  else if (numPulsesC > bcPosition) {
    Set_PWMC(-SPEED);
  }
  //numPulsesC < minPulses
  else {

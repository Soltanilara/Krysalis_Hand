...
void requestEvent() {
  int abcPosition = (int) (maxPulsesMiddleRingPinky * percentage);
  int dPosition = (int) (maxPulsesIndex * percentage);

  bool aComplete = abs(numPulsesA - abcPosition) < 20;
  bool bComplete = abs(numPulsesB - abcPosition) < 20;
  bool cComplete = abs(numPulsesC - abcPosition) < 20;
  bool dComplete = abs(numPulsesD - dPosition) < 20;

  Serial.println(aComplete && bComplete && cComplete & dComplete);
...
void setPosition() {
  ...
  if (abs(numPulsesA - abcPosition) < 20) {
    Set_PWMA(0);
    // Serial.println();
  }
  else if (numPulsesA > abcPosition) {
    Set_PWMA(-SPEED);
  }
  ...
  if (abs(numPulsesB - abcPosition) < 20) {
    Set_PWMB(0);
    // Serial.println();
  }
  else if (numPulsesB > abcPosition) {
    Set_PWMB(-SPEED);
  }
  //numPulsesC < minPulses
  else {
  ...
  if (abs(numPulsesC - abcPosition) < 20) {
    Set_PWMC(0);
  }
  else if (numPulsesC > abcPosition) {
    Set_PWMC(-SPEED);
  }
  //numPulsesC < minPulses
  else {
  ...
  if (abs(numPulsesD - dPosition) < 20) {
    Set_PWMD(0);
  }
  else if (numPulsesD > dPosition) {
    Set_PWMD(-SPEED);
  }

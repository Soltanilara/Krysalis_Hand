...
#define PIP_INTERRUPT 20
#define DIP_INTERRUPT 21

volatile bool PIP_Complete = false;
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(PIP_INTERRUPT), completePIP, RISING);
  attachInterrupt(digitalPinToInterrupt(DIP_INTERRUPT), completeDIP, RISING);
...
void loop() {
  switch (currentState) {
    case CONTRACT:
      if (contract() && PIP_Complete && DIP_Complete) {
        delay(1000);
        PIP_Complete = false;
        DIP_Complete = false;
        currentState = EXTEND;
      ...
      break;
    case EXTEND:
      if (extend() && PIP_Complete && DIP_Complete) {
        delay(1000);
        PIP_Complete = false;
        DIP_Complete = false;

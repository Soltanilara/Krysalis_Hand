...
#define maxPulsesIndexPinky 2400
#define maxPulsesMiddleRing 1800
...
bool setPosition(double percent) {
  ...
  int positionIndexPinky = (int) (maxPulsesIndexPinky * percent);
  bool incomplete = false;

  if (abs(numPulsesA - positionIndexPinky) <= 5) {
    Set_PWMA(0);
  }
  else if (numPulsesA <= positionIndexPinky) {
    Set_PWMA(SPEED);
    incomplete = true;
  }
  else {
  ...
  if (abs(numPulsesD - positionIndexPinky) <= 5) {
    Set_PWMD(0);
  }
  else if (numPulsesD <= positionIndexPinky) {
    Set_PWMD(SPEED);
    incomplete = true;
  }
  else {
